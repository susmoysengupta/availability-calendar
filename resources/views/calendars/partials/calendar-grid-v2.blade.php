<section class="p-8 mb-4 bg-white rounded-lg shadow-md dark:bg-gray-800">
    <div class="flex items-center justify-center row-start-1 lg:col-start-2">
        <div class="w-full max-w-xs shadow-lg">
            <div class="p-5 bg-white border rounded-t dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <template x-for="day,dayNo in week" :key="dayNo">
                                        <td class="p-0 text-center border border-gray-300">
                                            <template x-if="day.date != null">
                                                <div class="relative flex items-center justify-center w-full h-full cursor-pointer" @click="changeLegend(day.date)">
                                                    <p :id="`date-${day.date}`" x-text="day.date" tabindex="0" class="p-1.5 text-xs font-medium text-gray-700 select-none z-10"></p>
                                                    <div :id="`calendar-date-color-${day.date}`" class="absolute w-full h-full">
            ...
            <!-- Legends -->
            @if (auth()->user()->hasDirectPermission('update-legend'))
                <div class="grid grid-cols-1 gap-4 p-5 rounded-b dark:bg-gray-700 bg-gray-50">
                    <div class="flex justify-between w-full text-sm">
                        <p class="text-base font-semibold leading-relaxed text-gray-600 dark:text-gray-300">Legends</p>
                        <a href="{{ route('calendars.legends.index', $calendar) }}" class="text-purple-500 dark:text-purple-400">Edit</a>
                    </div>
                        ...
                        <template x-for="legend in legends" :key="legend.id">
                            <li class="flex items-center gap-2">
                                <div class="relative w-5 h-5 overflow-hidden text-gray-600 rounded dark:text-gray-100">
                                    <template x-if="legend.gradient != null">
                                ...
                                <span x-text="legend.title" class="text-xs font-semibold text-gray-600 dark:text-gray-400"></span>
                            </li>
                        </template>
                    </ul>
                </div>
